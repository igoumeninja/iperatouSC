/* IZ 100517
Synchronize processes running on different computers in a network via OSC. 
SyncSender uses a pattern to broadcast beats or other events to a list of NetAddr clients. 
SyncResponder listens to messages received from SyncSender via the net and binds actions to them. 

* Add parameters to TempoClock.new(1, 0) for start beat control AB 101031
*/

/*
a = SyncSender(Pbind.new);
a.start;

Stasis

b = SyncAction('beats', { | ... args | args.postln; });
b.activate;
b.deactivate;

a.clients = a.clients add: NetAddr.localAddr;
c = SyncAction('beats', { | ... args | args.postln })

b = SyncResponder.new;
b.addDependant({ | ... args | args.postln; });
c = SimpleController(b, 'beats', { | receiver, message, time |
...
SyncSender {
	...
	*new { | pattern, clients, clockFunc, protoEvent |
		...
		^this.newCopyArgs(pattern, (clients ? [NetAddr.localAddr, NetAddr("192.168.1.68", 57120), NetAddr("192.168.1.67", 57120)]), 
			clockFunc ?? {{ TempoClock.new(1, 0) }}, protoEvent).init;
	}
